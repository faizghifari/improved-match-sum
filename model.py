...
class MatchSum(nn.Module):
    ...
    def __init__(self, candidate_num, encoder, hidden_size=768):
        ...
        if encoder == 'bert':
            self.encoder = BertModel.from_pretrained('bert-base-german-cased') #self.encoder = BertModel.from_pretrained('bert-base-uncased')
        elif encoder == "roberta":
            self.encoder = RobertaModel.from_pretrained('roberta-base')
        elif encoder == "bert-meanmax":
            self.custom_bert_config = CustomBertConfig.from_pretrained(
                pretrained_model_name_or_path='bert-base-uncased',
                pooling_layer_type='MEAN_MAX',
            )
            self.encoder = CustomBertModel.from_pretrained('bert-base-uncased', config=self.custom_bert_config)
        elif encoder == "bert-lstm":
            self.custom_bert_config = CustomBertConfig.from_pretrained(
                pretrained_model_name_or_path='bert-base-uncased',
                pooling_layer_type='LSTM',
            )
            self.encoder = CustomBertModel.from_pretrained('bert-base-uncased', config=self.custom_bert_config)
        elif encoder == "bert-gru":
            self.custom_bert_config = CustomBertConfig.from_pretrained(
                pretrained_model_name_or_path='bert-base-uncased',
                pooling_layer_type='GRU',
            )
            self.encoder = CustomBertModel.from_pretrained('bert-base-uncased', config=self.custom_bert_config)
        else:
            raise Exception("Unknown encoder: {}".format(encoder))
