...
def configure_training(args):
    ...
    params['encoder_model'] = args.encoder_model
...
known_encoder = ['bert', 'roberta', 'distilbert']
def verify_encoder(encoder):
    return encoder in known_encoder

def adjust_args_encoder(args):
    if args.encoder == 'bert-meanmax' or args.encoder == 'bert-lstm' or args.encoder == 'bert-gru':
        args.encoder_model = args.encoder
        args.encoder = 'bert'
    else:
        args.encoder_model = args.encoder

    assert verify_encoder(args.encoder), "unknown encoder: {}".format(args,encoder)
    return args

def train_model(args):

    args = adjust_args_encoder(args)

    # check if the data_path and save_path exists
    data_paths = get_data_path(args.mode, args.encoder, args.lang)
    ...
    # configure model
    model = MatchSum(args.candidate_num, args.encoder_model)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lang', default='en',
                        help='en for cnn/dm, de for MLSUM/de', type=str)
